type Testimonial = {
  quote: string;
  attribution: string;
  initials: string;
};

const testimonials: Testimonial[] = [
  {
    quote:
      "Operavo AI helped us respond to inbound leads instantly without expanding our front desk operations.",
    attribution: "Real Estate Operations Team",
    initials: "RE",
  },
  {
    quote:
      "The voice agent handled qualification and appointment routing far better than we expected during our pilot.",
    attribution: "Brokerage Partner",
    initials: "BP",
  },
  {
    quote:
      "We saw immediate improvement in lead response consistency after implementing Operavo AI.",
    attribution: "Sales Director",
    initials: "SD",
  },
  {
    quote:
      "The platform helped reduce missed opportunities from after-hours inbound calls.",
    attribution: "Real Estate Growth Team",
    initials: "RG",
  },
  {
    quote:
      "Operavo AI streamlined how we handled incoming inquiries and appointment scheduling.",
    attribution: "Property Sales Team",
    initials: "PS",
  },
  {
    quote:
      "Simple setup, professional call handling, and reliable lead capture.",
    attribution: "Real Estate Agency Partner",
    initials: "RA",
  },
];

function TestimonialCard({
  t,
  ariaHidden,
}: {
  t: Testimonial;
  ariaHidden?: boolean;
}) {
  return (
    <figure
      aria-hidden={ariaHidden}
      className="group relative flex w-[86vw] shrink-0 flex-col overflow-hidden rounded-3xl border border-white/6 bg-linear-to-br from-emerald-950/15 via-zinc-950/50 to-black p-7 transition-all duration-500 ease-out hover:-translate-y-1 hover:border-white/12 hover:shadow-[0_8px_30px_rgba(0,0,0,0.4)] sm:w-[360px] lg:w-[384px]"
    >
      <div
        aria-hidden
        className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_30%_30%,rgba(16,185,129,0.08),transparent_60%)] opacity-0 transition-opacity duration-500 group-hover:opacity-100"
      />

      <svg
        className="relative z-10 h-7 w-7 text-emerald-400/70"
        viewBox="0 0 24 24"
        fill="currentColor"
        aria-hidden
      >
        <path d="M9.4 6.5c-3 1.3-5.4 3.8-5.4 7.8V18h6v-6H6.7c.3-2.2 1.8-3.7 4-4.6L9.4 6.5zm10 0c-3 1.3-5.4 3.8-5.4 7.8V18h6v-6h-3.3c.3-2.2 1.8-3.7 4-4.6L19.4 6.5z" />
      </svg>

      <blockquote className="relative z-10 mt-5 flex-1 text-[15px] leading-[1.7] text-zinc-200">
        {t.quote}
      </blockquote>

      <figcaption className="relative z-10 mt-7 flex items-center gap-3 border-t border-white/5 pt-5">
        <span
          aria-hidden
          className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-emerald-500/10 text-[12px] font-semibold text-emerald-300 ring-1 ring-inset ring-emerald-500/20"
        >
          {t.initials}
        </span>
        <div className="min-w-0">
          <p className="truncate text-sm font-medium text-white">
            {t.attribution}
          </p>
        </div>
      </figcaption>
    </figure>
  );
}

export function Testimonials() {
  return (
    <section className="relative mx-auto w-full max-w-7xl px-4 py-28 sm:px-6 md:py-36">
      <div
        aria-hidden
        className="pointer-events-none absolute inset-x-0 top-[10%] -z-10 h-[500px] bg-[radial-gradient(ellipse_50%_50%_at_50%_50%,rgba(16,185,129,0.05),transparent_80%)] mix-blend-screen"
      />

      <div className="mx-auto max-w-3xl text-center">
        <div className="flex items-center justify-center gap-3">
          <span className="h-px w-8 bg-emerald-400/60" />
          <p className="text-[11px] font-medium uppercase tracking-[0.15em] text-zinc-300">
            From the line
          </p>
          <span className="h-px w-8 bg-emerald-400/60" />
        </div>
        <h2 className="mt-5 text-balance text-3xl font-medium leading-[1.15] -tracking-[0.02em] text-white sm:text-4xl md:text-5xl">
          Real teams,{" "}
          <span className="font-accent italic font-normal text-zinc-400">
            real outcomes.
          </span>
        </h2>
        <p className="mx-auto mt-5 max-w-xl text-[15px] leading-[1.6] font-light text-zinc-400">
          Independent agents and brokerages running Operavo on their inbound
          line.
        </p>
      </div>

      <style>{`
        @keyframes operavo-marquee {
          from { transform: translateX(0); }
          to { transform: translateX(-50%); }
        }
        .operavo-marquee-track {
          animation: operavo-marquee 45s linear infinite;
        }
        .operavo-marquee-track:hover {
          animation-play-state: paused;
        }
        @media (prefers-reduced-motion: reduce) {
          .operavo-marquee-track { animation: none; }
        }
      `}</style>

      <div className="mt-14 overflow-hidden [mask-image:linear-gradient(to_right,transparent,black_6%,black_94%,transparent)]">
        <div className="operavo-marquee-track flex w-max items-stretch gap-4 pr-4">
          {testimonials.map((t) => (
            <TestimonialCard key={t.attribution} t={t} />
          ))}
          {testimonials.map((t) => (
            <TestimonialCard key={`${t.attribution}-dup`} t={t} ariaHidden />
          ))}
        </div>
      </div>
    </section>
  );
}
