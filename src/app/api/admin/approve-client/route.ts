...
export async function POST(req: Request) {
    ...
    return NextResponse.json(
      {
        ok: true,
        profile: updated,
        provision: { ok: false, error: "N8N_PROVISION_WEBHOOK_URL is not set" },
      },
      { status: 200 }
    );
  }
  ...
  try {
    const provisionRes = await fetch(webhookUrl, {
      ...
      body: JSON.stringify({ tenant_id: updated.tenant_id }),
    });
    const provisionJson = (await provisionRes.json().catch(() => ({}))) as Record<
      string,
      unknown
    >;
    return NextResponse.json({
      ok: true,
      profile: updated,
      provision: { ok: provisionRes.ok, status: provisionRes.status, ...provisionJson },
    });
  } catch (err) {
    return NextResponse.json({
      ok: true,
      profile: updated,
      provision: { ok: false, error: err instanceof Error ? err.message : String(err) },
    });
  }
}
